...
interface BitcoinRiskChartProps {
  ...
  isCollapsed?: boolean;
  setSelectedRange?: (range: string) => void; // If needed for changes
}

const BubbleChart: React.FC<BitcoinRiskChartProps> = ({ onBubbleClick, selectedRange, isCollapsed, setSelectedRange }) => {
  ...
  const handleRangeChange = (range: string) => {
    if (setSelectedRange) setSelectedRange(range);
  ...
  return (
    <>
      {/* Filter UI at top (like BuySignalsPanel) */}
      <div className="relative dropdown-container mb-2 flex justify-center">
        <button
        ...
        >
          {selectedRange}
          {/* Use a small icon if needed, or remove entirely */}
          {/* <ChevronDown size={20} /> */}
        </button>
        {showRankDropdown && (
          <div className="absolute top-full mt-1 w-48 bg-gray-800 rounded-lg shadow-lg z-50">
